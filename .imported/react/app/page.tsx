...
function TechnologyCard({ technology, selected, onAdd }: { technology: Technology; selected: boolean; onAdd: () => void }) {
  return (
    <article className="flex min-h-[148px] flex-col rounded-xl border border-slate-100 bg-white p-3 sm:min-h-[238px] sm:rounded-2xl sm:p-5 shadow-[0_3px_14px_rgba(24,39,75,0.04)] transition hover:-translate-y-0.5 hover:shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <img src={technology.icon} alt="" className="h-5 w-5 object-contain sm:h-7 sm:w-7" />
        <span className="rounded-full bg-sky-50 px-3 py-1 text-[10px] font-semibold text-sky-500">{technology.badge}</span>
      </div>
      ...
      <button disabled={selected} onClick={onAdd} className="mt-3 h-8 rounded-md bg-slate-950 text-xs font-medium text-white transition hover:bg-slate-800 disabled:cursor-default disabled:bg-emerald-50 disabled:text-emerald-600">
